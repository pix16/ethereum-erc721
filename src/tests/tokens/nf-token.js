...
describe('nf-token', function() {
  ...
  it('correctly mints a NFT', async function() {
    expect(await nfToken.connect(owner).transferFrom(owner.address,bob.address, 1)).to.emit(nfToken, 'Transfer');
  ...
  it('returns correct balanceOf', async function() {
    ...
    expect(await nfToken.balanceOf(bob.address)).to.equal(1);
    await nfToken.connect(owner).transferFrom(owner.address,bob.address, id2);
  ...
  it('throws when trying to transfer 2 NFTs with the same ids', async function() {
    await nfToken.connect(owner).transferFrom(owner.address,bob.address, id1);
    await expect(nfToken.connect(owner).transferFrom(owner.address,bob.address, id1)).to.be.revertedWith('003004');
  });

  it('throws when trying to mint NFT to 0x0 address', async function() {
    await expect(nfToken.connect(owner).transferFrom(owner.address,zeroAddress, id1)).to.be.revertedWith('003001');
  });
